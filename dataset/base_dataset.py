...
_T_Vision = TypeVar("_T_Vision")


class _T_Vision_Dataset(Protocol):
    ...
    spatial_transform: Optional[Callable[[_T_Vision], _T_Vision]]
    image_transform: Optional[Callable[[_T_Vision], _T_Vision]]
    label_transform: Optional[Callable[[_T_Vision], _T_Vision]]


@dataclass
class _VisionBaseDataset(Dataset, _T_Vision_Dataset, ABC):
    """
    ...
    io: IOProtocol
    dataset_type: Literal["train", "val", "test"]
    pixel_mean: Tuple[float, float, float] = field(default=(0.5, 0.5, 0.5))
    pixel_std: Tuple[float, float, float] = field(default=(0.5, 0.5, 0.5))
    image_size: Tuple[int, int] = field(default=(256, 256))
    spatial_transform: Optional[Callable[[_T_Vision], _T_Vision]] = None
    image_transform: Optional[Callable[[_T_Vision], _T_Vision]] = None
    label_transform: Optional[Callable[[_T_Vision], _T_Vision]] = None
    image_list: list[tuple[str, str]] = field(default_factory=list, init=False)
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        [image, label] = self.load(path_list)  # type: ignore
        image, label = self.total_transform(image, label)
        return image, label

    def total_transform(
        self, image: _T_Vision, label: _T_Vision
    ) -> tuple[_T_Vision, _T_Vision]:
...
class _VisionBaseIncrement(_VisionBaseDataset, ABC):
    stage: int
    label_strategy: Literal["current", "full", "history"]
    incremental_transform: Callable[[_T_Vision], _T_Vision]

    @abstractmethod
    def update_task(self, stage_labels: list[int]) -> None:
        ...
...
class VisionDataset(_VisionBaseIncrement):
    def _get_path(self) -> None:
        pass
...
if __name__ == "__main__":
    ...
    #
    # transform = T.Compose([T.Resize(512), T.RandomCrop(256), T.ToTensor()])
    # image = IO.load(r"../test_data/101313650_p0.png")
    # print(image)
    # tensor = transform(image)
    # print("finish")
    test = _VisionMetaData(root="", io=IO, dataset_type="test")
    print(test)
