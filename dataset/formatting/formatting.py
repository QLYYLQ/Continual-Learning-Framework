...
from CLTrainingFramework.dataset.schema.Schema import Schema
from CLTrainingFramework.dataset.arrow_utils import _ArrayXDExtensionType, _is_zero_copy_only, pandas_types_mapper
from  CLTrainingFramework.dataset.schema.load_file import load_from_storage_with_nested_sample
from CLTrainingFramework.dataset.arrow_handler.arrow_table.block_table  import Table
...
class PythonFeaturesDecoder:
    def __init__(
        self, schema: Optional[Schema], token_per_repo_id: Optional[dict[str, Union[str, bool, None]]] = None
    ):
        self.schema =schema
...
class PandasFeaturesDecoder:
    ...
    def decode_row(self, row: pd.DataFrame) -> pd.DataFrame:
        decode = (
            {
                column_name: no_op_if_value_is_null(partial(load_from_storage_with_nested_sample, feature))
                for column_name, feature in self.schema.items()
                if self.schema._column_requires_decoding[column_name]
            }
            if self.schema
            else {}
        )
...
class Formatter(Generic[RowFormat, ColumnFormat, BatchFormat]):
    ...
    def __init__(
        ...
        self.schema = schema
        self.token_per_repo_id = token_per_repo_id
        self.python_features_decoder = PythonFeaturesDecoder(self.schema, self.token_per_repo_id)
        self.pandas_features_decoder = PandasFeaturesDecoder(self.schema)
...
class PythonFormatter(Formatter[Mapping, list, Mapping]):
    def __init__(self, schema=None, lazy=False, token_per_repo_id=None):
        super().__init__(schema, token_per_repo_id)
        self.lazy = lazy
    ...
    def format_row(self, pa_table: pa.Table) -> Mapping:
        ...
        row = self.python_features_decoder.load_from_storage_with_row(row)
        return row

    def format_column(self, pa_table: pa.Table) -> list:
        column = self.python_arrow_extractor().extract_column(pa_table)
        column = self.python_features_decoder.load_from_storage_with_column(column, pa_table.column_names[0])
        return column
    ...
    def format_batch(self, pa_table: pa.Table) -> Mapping:
        ...
        batch = self.python_features_decoder.load_from_storage_with_batch(batch)
        return batch
...
class CustomFormatter(Formatter[dict, ColumnFormat, dict]):
    ...
    def __init__(self, transform: Callable[[dict], dict], schema=None, token_per_repo_id=None, **kwargs):
        super().__init__(schema=schema, token_per_repo_id=token_per_repo_id)
...
def format_table(
    ...
    python_formatter = PythonFormatter(schema=formatter.schema)
