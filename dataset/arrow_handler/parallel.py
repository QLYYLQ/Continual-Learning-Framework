...
def parallel_map(function, iterable, num_proc, batched, batch_size, types, disable_tqdm, desc, single_map_nested_func):
    """
    Apply a function to iterable elements in parallel, where the implementation uses either
...
def parallel_backend(backend_name: str):
    """
     Configures the parallel backend for parallelized dataset loading, which uses the parallelization
